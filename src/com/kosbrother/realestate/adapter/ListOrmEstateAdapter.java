...
public class ListOrmEstateAdapter  extends BaseAdapter
{

	private final Activity activity;
	private final ArrayList<OrmRealEstate> data;
	private static LayoutInflater inflater = null;
	private ImageLoader imageLoader;

	// private String mChannelTitle;

	public ListOrmEstateAdapter(Activity a, ArrayList<OrmRealEstate> d)
	{
		activity = a;
		data = d;
	...
	public View getView(final int position, View convertView, ViewGroup parent)
	{
		...
		TextView textHouseAge = (TextView) vi
				.findViewById(R.id.text_info_house_age);
		TextView textBuyDate = (TextView) vi
				.findViewById(R.id.text_info_buy_date);

		TextView textGroundArea = (TextView) vi
				.findViewById(R.id.text_info_ground_area);
		TextView textEstateType = (TextView) vi
				.findViewById(R.id.text_info_estate_type);

		TextView textTotalPrice = (TextView) vi
				.findViewById(R.id.text_info_total_price);
		TextView textBuildingType = (TextView) vi
		...
		textHouseAge
				.setText(InfoParserApi.parseHouseAge(data.get(position).date_built));
		textBuyDate
				.setText(InfoParserApi.parseBuyDate(data.get(position).date_buy));

		textGroundArea.setText(InfoParserApi.parseBuildingExchangeArea(data
				.get(position).building_exchange_area));
		textEstateType
				.setText(InfoParserApi.parseEstateType(data.get(position).estate_type));

		textTotalPrice.setText(InfoParserApi.parseTotalBuyMoney(data
				.get(position).buy_total_price));
		textBuildingType.setText(InfoParserApi.parseEstateType(data
				.get(position).building_type));

		textBuyPerSquareFeet.setText(InfoParserApi.parsePerSquareFeetMoney(data
		...
		textRooms.setText(Integer.toString(data.get(position).building_room)
				+ "房"
				+ Integer.toString(data.get(position).building_sitting_room)
				+ "廳" + Integer.toString(data.get(position).building_rest_room)
				+ "衛浴");
